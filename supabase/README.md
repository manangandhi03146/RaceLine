...
18. `migrations/018_avatar_storage.sql` — Adds storage policies for the `avatars` bucket. **Before running, create the `avatars` bucket in the Supabase Dashboard and mark it PUBLIC.** Writes are still owner-only (path prefix must match `auth.uid()`); reads are open so we don't need a signed-URL round-trip per render.
